...
DATATYPE.prototype.read = function (cursor) {
  ...
  if (wf==='String') {
    ...
    if (this.extent === 'counted') {
      var countedType = new NumberType('counted', {type: 'Number', bytes: this.bytes, endian: this.endian, field: 'unsigned'});
...
DATATYPE.prototype.toQvxSpec = function () {
  ...
  if (this.format) {
    ...
    if (typeof this.format.nDec !== 'unknown') { obj.FieldFormat.nDec = this.format.nDec; }
    if (typeof this.format.useThou !== 'unknown') { obj.FieldFormat.UseThou = this.format.useThou; }
    if (typeof this.format.decSep !== 'unknown') { obj.FieldFormat.Dec = this.format.decSep; }
    if (typeof this.format.thouSep !== 'unknown') { obj.FieldFormat.Thou = this.format.thouSep; }
  }
