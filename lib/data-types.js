...
DATATYPE.prototype.read = function () {
  throw new errors.NotImplementedError(this.key + '.read()');
}
...
var NUMBER = function(options) {
  ...
  if ([undefined, 8, 16, 32, 64].indexOf(this._length) === -1) {
...
Object.defineProperty(INTEGER.prototype, 'wireFormat', {
  get: function () {
    return (this.options.unsigned ? 'U': '') + 'Int' + options.length +
...
Object.defineProperty(FLOAT.prototype, 'wireFormat' , {
  get: function () {
    return (this._length === 64 ? 'Double' : 'Float') + (this._bigEndian ? 'BE' : 'LE');
  }
});
...
var STRING = function(encoding, length, countedEndian) {
  ...
  this.counted = function (endian) {
    ...
    this.sizeType = INTEGER(this._length, this._countedEndian === 'BE').UNSIGNED;
  };
...
STRING.prototype.counted = function (endian) {
  this._countedEndian = endian;
  this.options.countedEndian = endian;
}
