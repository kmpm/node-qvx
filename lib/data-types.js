...
//     textLength = countedType.read(cursor);
  ...
  try {
    ...
    debug('%d %s(%s/%s)>ERROR at %s', start, this.name, wf, this.type, cursor.tell());
    throw err;
  }
  return v;
};
...
DATATYPE.prototype.write = function (cursor, value) {
  ...
  if (wf === 'String') {
    ...
    if (this.extent === 'counted') {
      ...
      cursor.write(value, value.length, this.encoding);
...
DualType.prototype.read = function (cursor) {
  ...
  if (typeof v === 'object' && v) {
    if (v.flag === 6 && v.value instanceof Array) {
      //if first is a number, return it.
      if (isNaN(v.value[0])) {
        return v.value[1];
      }
      return v.value[0];
