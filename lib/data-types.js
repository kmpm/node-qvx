...
DATATYPE.prototype.write = function (cursor, value) {
  ...
  if (wf === 'String') {
    ...
    if (this.extent === 'counted') {
      ...
      var rc = cursor.copy(value);
      var diff = this.bytes - value.length;
      if (diff > 0) {
        debug('fill %d', diff);
        rc = cursor.fill(0, diff);
      }
      debug('written fix');
      return rc;
    }
...
DateType.prototype.read = function (cursor) {
  ...
  debug('timezone=%s and format=%s using `%s`', this.timezone, this.format.fmt, v);
  if (v && v.length >= 1) {
    if (this.timezone) {
      v = moment.tz(v, this.format.fmt, this.timezone).toDate();
    }
    else {
    ...
    debug('not a parsable value', v);
    v = null;
  }
  return v;
};

DateType.prototype.write = function (cursor, value) {
  ...
  DATATYPE.prototype.write.call(this, cursor, value);
};
