...
var QvxCursor = require('./qvxcursor');
...
//       }
//     }
//     else if(f.NullRepresentation === 'QVX_NULL_NEVER') {
//       //do not allow null
//     }
//     else {
...
LineReader.prototype.parse = function (buf) {
  ...
  if(typeof buf !== 'undefined') {
    if (this.bc === null) {
      debug('new cursor');
      this.bc = new QvxCursor(buf);
    }
    else {
      debug('concat cursor');
      var oldIndex = this.bc.tell();
      this.bc = new QvxCursor(Buffer.concat([this.bc.buffer, buf]));
      this.bc.seek(oldIndex);
    }
    debug('prepping fields');
    this.fields.forEach(function (f) {
      f.read = self.bc.getReader(f);
    });
  }
  var bc = this.bc;
  var fieldIndex = 0;
